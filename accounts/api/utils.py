...
PROBLEM_CATALOG: dict[int, tuple[str, str]] = {
    ...
    418: (
        "I'm a teapot",
        "Запрос не может быть обработан: намерение клиента не соответствует возможностям API WAF.",
    ),
    429: (
        "Too Many Requests",
...
def resolve_request_id(request=None, request_id: str | None = None) -> str:
    if request_id:
        return request_id
...
def problem_response(
    ...
    *,
    ...
    Параметры:
        ...
        request / request_id — единый идентификатор запроса (UUIDv7)
    """
    default_title, default_detail = PROBLEM_CATALOG.get(
        status, ("Error", "Произошла ошибка.")
    )

    rid = resolve_request_id(request=request, request_id=request_id)
...
def problem_json_response(
    ...
    *,
    ...
    """Problem Details as Django JsonResponse (for middleware)."""
    default_title, default_detail = PROBLEM_CATALOG.get(
        status, ("Error", "Произошла ошибка.")
    )
    rid = resolve_request_id(request=request, request_id=request_id)
