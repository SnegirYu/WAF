...
from accounts.api.authentication import AccessTokenAuthentication
...
class AdminTokenProtectedAPIView(TokenProtectedAPIView):
...
class LogoutView(SessionProtectedAPIView):
...
@extend_schema(tags=["Auth"])
class Setup2FAView(SessionProtectedAPIView):
...
class Verify2FASetupView(SessionProtectedAPIView):
...
class SiteListCreateView(TokenProtectedAPIView):
    def get(self, request: Request) -> Response:
...
@extend_schema(parameters=[_AUTH_HEADER_PARAM])
class SiteDetailView(TokenProtectedAPIView):
...
@extend_schema(parameters=[_AUTH_HEADER_PARAM])
class RuleListCreateView(AdminTokenProtectedAPIView):
    def get(self, request: Request) -> Response:
...
@extend_schema(parameters=[_AUTH_HEADER_PARAM])
class RuleDetailView(AdminTokenProtectedAPIView):
...
@extend_schema(parameters=[_AUTH_HEADER_PARAM])
class RuleToggleView(AdminTokenProtectedAPIView):
...
@extend_schema(parameters=[_AUTH_HEADER_PARAM])
class LogListView(TokenProtectedAPIView):
    def get(self, request: Request) -> Response:
        user = request.token.user
...
@extend_schema(parameters=[_AUTH_HEADER_PARAM])
class LogExportView(TokenProtectedAPIView):
    def post(self, request: Request) -> Response:
        user = request.token.user
        try:
...
@extend_schema(parameters=[_AUTH_HEADER_PARAM])
class LogExportListView(TokenProtectedAPIView):
    def get(self, request: Request) -> Response:
        user = request.token.user
...
@extend_schema(parameters=[_AUTH_HEADER_PARAM])
class LogExportDeleteView(TokenProtectedAPIView):
...
@extend_schema(parameters=[_AUTH_HEADER_PARAM])
class TokenListCreateView(TokenProtectedAPIView):
    def get(self, request: Request) -> Response:
        user = request.token.user
        qs = (
...
@extend_schema(parameters=[_AUTH_HEADER_PARAM])
class TokenDetailView(TokenProtectedAPIView):
...
@extend_schema(parameters=[_AUTH_HEADER_PARAM])
class TokenBlockIPView(AdminTokenProtectedAPIView):
...
@extend_schema(parameters=[_AUTH_HEADER_PARAM])
class TokenUnblockIPView(AdminTokenProtectedAPIView):
    def post(self, request: Request, token_id: int) -> Response:
        try:
            token = AccessToken.objects.get(pk=token_id)
        except AccessToken.DoesNotExist:
...
@extend_schema(parameters=[_AUTH_HEADER_PARAM])
class UserListView(AdminTokenProtectedAPIView):
    def get(self, request: Request) -> Response:
...
@extend_schema(parameters=[_AUTH_HEADER_PARAM])
class UserDetailView(AdminTokenProtectedAPIView):
...
@extend_schema(
    ...
    responses={
        ...
        401: _ERR_401,
        403: _ERR_403,
        503: OpenApiResponse(response=ProblemDetailSerializer, description="Сервис недоступен (БД)."),
        500: _ERR_500,
    },
)
@extend_schema(parameters=[_AUTH_HEADER_PARAM])
class MonitoringView(AdminTokenProtectedAPIView):
    def get(self, request: Request) -> Response:
        try:
            data = get_monitoring_metrics()
            if not data.get("db_ok", True):
                return problem_response(
                    503,
                    detail="Сервис временно недоступен: нет подключения к базе данных.",
                    request=request,
                )
...
@extend_schema(parameters=[_AUTH_HEADER_PARAM])
class BannedIPListView(AdminTokenProtectedAPIView):
    def get(self, request: Request) -> Response:
...
@extend_schema(parameters=[_AUTH_HEADER_PARAM])
class AdminMessageListView(AdminTokenProtectedAPIView):
    def get(self, request: Request) -> Response:
...
@extend_schema(parameters=[_AUTH_HEADER_PARAM])
class AdminMessageReadView(AdminTokenProtectedAPIView):
...
@extend_schema(parameters=[_AUTH_HEADER_PARAM])
class AdminMessageCreateView(TokenProtectedAPIView):
    def post(self, request: Request) -> Response:
...
@extend_schema(parameters=[_AUTH_HEADER_PARAM])
class SessionListView(AdminTokenProtectedAPIView):
    def get(self, request: Request) -> Response:
        now = timezone.now()
...
@extend_schema(parameters=[_AUTH_HEADER_PARAM])
class SessionDeleteView(AdminTokenProtectedAPIView):
...
@extend_schema(parameters=[_AUTH_HEADER_PARAM])
class StatsView(TokenProtectedAPIView):
    def get(self, request: Request) -> Response:
...
@extend_schema(parameters=[_AUTH_HEADER_PARAM])
class AdminPasswordChangeView(AdminTokenProtectedAPIView):
    def post(self, request: Request) -> Response:
...
@extend_schema(parameters=[_AUTH_HEADER_PARAM])
class UserPasswordChangeView(SessionProtectedAPIView):
...
@extend_schema(parameters=[_AUTH_HEADER_PARAM])
class AdminAvatarView(AdminTokenProtectedAPIView):
...
@extend_schema(parameters=[_AUTH_HEADER_PARAM])
class UserAvatarView(SessionProtectedAPIView):
    parser_classes = [MultiPartParser, FormParser]
