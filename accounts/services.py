...
def add_site_for_user(user, form):
    if not user.can_add_site():
        limit = user.get_sites_limit()
        plan = user.get_tariff_limits()['label']
        raise ValueError(
            f"Достигнут лимит тарифного плана «{plan}»: максимум {limit} "
            f"{'сайт' if limit == 1 else 'сайтов' if limit in (2,3,4) else 'сайтов'}. "
...
def delete_site_for_user(user, site_id: int) -> str:
    """Удаляет сайт пользователя. Возвращает домен удалённой записи."""
    site = ProtectedSite.objects.get(pk=site_id, user=user)
    domain = site.domain
    site.delete()
    return domain
